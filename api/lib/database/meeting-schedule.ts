...
export async function getMeetingSchedule(
  ...
  const sql = {
    text: `
      SELECT s.id, s.meeting_id, s.schedule_attr,
...
export async function listMeetingScheduleByMeeting(
  ...
  const sql = {
    text: `
      SELECT id, meeting_id, schedule_attr, host_key,
        (SELECT started_at
...
export async function addMeetingSchedule(
  ...
  const sql = {
    text: `
      INSERT INTO meeting_schedule (meeting_id, schedule_attr)
      VALUES (
        (SELECT id
         FROM meeting
         WHERE id = $2
...
// -----------------------------------------------------------------------------
export async function resetMeetingScheduleHostKey(
  identityId: string,
  scheduleId: string,
  isSuperAdmin = false,
) {
