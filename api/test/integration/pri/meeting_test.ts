...
describe("pri/meeting", () => {
  ...
  it("adds a new meeting", async () => {
    ...
    assertEquals(body[0].name, "My Meeting");
  });
  ...
  it("gets a meeting by id", async () => {
    ...
    const req = makeRequest("POST", "/api/pri/meeting/get", { id: meetingId });
  ...
  it("updates a meeting", async () => {
    ...
    const res = await routeMeeting(req, "/api/pri/meeting/update", identityId);
    assertEquals(res.status, 200);
    const body = await res.json();
    assertEquals(Array.isArray(body), true);
    assertEquals(body[0].name, "After Update");
  });
  ...
  it("enables a meeting", async () => {
    ...
    const res = await routeMeeting(req, "/api/pri/meeting/enable", identityId);
    assertEquals(res.status, 200);
    const body = await res.json();
    assertEquals(Array.isArray(body), true);
    assertEquals(body[0].enabled, true);
  });
  ...
  it("disables a meeting", async () => {
    ...
    const body = await res.json();
    assertEquals(Array.isArray(body), true);
    assertEquals(body[0].enabled, false);
  });
