  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto', marginTop: 8 }}>
      <Form layout="vertical" onFinish={onFinish}>
        <FormText
          name="mailer_host"
          label={t('form.mailer_host')}
          value={p.mailer_host}
          onChange={(v) => set('mailer_host', v)}
        />
        <FormText
          name="mailer_port"
          label={t('form.mailer_port')}
          value={p.mailer_port}
          onChange={(v) => set('mailer_port', v)}
        />
        <FormSwitch
          label={t('form.mailer_secure')}
          checked={p.mailer_secure}
          onChange={(v) => set('mailer_secure', v)}
        />
        <FormText
          name="mailer_user"
          label={t('form.mailer_user')}
          value={p.mailer_user}
          onChange={(v) => set('mailer_user', v)}
        />
        <FormPassword
          name="mailer_pass"
          label={t('form.mailer_pass')}
          value={p.mailer_pass}
          onChange={(v) => set('mailer_pass', v)}
          hint={t('form.secret_hint')}
        />
        <FormText
          name="mailer_from"
          label={t('form.mailer_from')}
          value={p.mailer_from}
          onChange={(v) => set('mailer_from', v)}
        />
