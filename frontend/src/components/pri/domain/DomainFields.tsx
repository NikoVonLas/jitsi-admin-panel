...
export default function DomainFields({
  ...
  const authTypeOptions = [
    { value: 'none', label: t('domain.auth_none') },
    { value: 'token', label: t('domain.auth_token') },
  ];
      ...
      {authType === 'token' ? (
        <>
          <FormUrl
            name="url"
            label={t('form.url')}
            value={domainAttr.url}
            onChange={(v) => setAttr('url', v)}
            required
            disabled={disabled}
          />
          <FormText
            name="app_id"
            label={t('form.app_id')}
            value={domainAttr.app_id}
            onChange={(v) => setAttr('app_id', v)}
            required
            disabled={disabled}
          />
          <FormPassword
            name="app_secret"
            label={t('form.app_secret')}
            value={domainAttr.app_secret}
            onChange={(v) => setAttr('app_secret', v)}
            required
            disabled={disabled}
          />
        </>
