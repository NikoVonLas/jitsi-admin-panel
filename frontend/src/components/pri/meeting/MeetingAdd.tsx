...
export default function MeetingAdd({ onCancel, onSuccess }: Props) {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        const enableds = roomItems
          .filter((r) => r.enabled && r.chain_enabled)
  ...
  const roomOptions: [string, string][] = [
    ...
    ...rawRooms.map((r): [string, string] => {
      ...
      return [
        r.id,
        `${r.label} — ${r.domain_url.replace(/^https?:\/\//, '')}/${r.name}${disabledSuffix}`,
      ];
    }),
  ];
  ...
  async function finalize() {
    try {
      ...
      if (roomId === NEW_ROOM) {
        const room = await action('/api/pri/room/add', {
          label: roomLabel,
          name: roomSlug,
          domain_id: roomDomainId,
          has_suffix: false,
        });
        finalRoomId = room.id;
      }
      ...
      await Promise.all(
        pendingSchedules.map((sa) =>
          action('/api/pri/meeting/schedule/add', { meeting_id: meeting.id, schedule_attr: sa })
        )
      );
      onSuccess?.();
    } catch {
  ...
  return (
    <Form layout="vertical">
      {step === 'meeting' && (
        <MeetingFields
          name={meetingName}
          onNameChange={setMeetingName}
          info={meetingInfo}
          onInfoChange={setMeetingInfo}
          roomId={roomId}
      ...
      {step === 'room' && (
        <>
          ...
          <RoomFields
            ...
            slug={roomSlug}
            onSlugChange={setRoomSlug}
            domainId={roomDomainId}
            onDomainIdChange={setRoomDomainId}
            hideDomain={hideDomainInRoom}
            disabled={disabled}
          />
        </>
      )}
      ...
      {step === 'schedule' && schedSubStep === 'choice' ? (
        ...
        <FormActions>
          <ButtonCancel
            onClick={cancel}
            disabled={disabled}
            label={step === 'meeting' ? t('btn.cancel') : t('sub.back')}
            block
          />
          <ButtonSubmit
            onClick={handleNext}
            disabled={disabled}
            label={step === 'schedule' ? t('btn.add_schedule') : t('btn.next')}
            htmlType="button"
            block
          />
        </FormActions>
