...
export default function CalendarMonthPage() {
  ...
  const monthName = (() => {
    try {
      return `${toLocaleMonthNameLong(date, lang)} ${date.slice(0, 4)}`;
    } catch {
      return date;
    }
  })();
  ...
  const syncUrl = syncToken
    ? `${globalThis.location.protocol}//${localStorage.getItem('galaxy_fqdn') || globalThis.location.host}/api/pub/ical/${syncToken}`
    : '';
  ...
  return (
    <div>
      <Subheader
        ...
        hrefMeeting="/meeting"
        hrefMeetingTitle={t('sub.meeting_list')}
      />
      <div
        style={{
          display: 'flex',
      ...
      >
        <Space>
          <Tooltip title={t('sub.prev_month')}>
            <Button
              icon={<i className="bi bi-chevron-left" />}
              onClick={() => navigate(`/calendar/month/${getDayOfPreviousMonth(date)}`)}
            />
          </Tooltip>
          <Tooltip title={t('sub.today')}>
            <Button
              icon={<i className="bi bi-calendar-check" />}
              onClick={() => navigate(`/calendar/month/${getToday()}`)}
            />
          </Tooltip>
          <Tooltip title={t('sub.next_month')}>
            <Button
              icon={<i className="bi bi-chevron-right" />}
              onClick={() => navigate(`/calendar/month/${getDayOfNextMonth(date)}`)}
            />
          </Tooltip>
        </Space>
        <Button icon={<i className="bi bi-calendar-check" />} onClick={openSyncModal}>
          {t('cal.sync')}
        </Button>
      </div>
            ...
            return (
              <>
                ...
                <div style={{ marginBottom: 16 }}>
                  <Button href={webcalUrl} icon={<i className="bi bi-calendar-plus" />}>
                    {t('cal.sync_subscribe')}
                  </Button>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <Button
                    ...
                    loading={syncRegenerating}
                    icon={<i className="bi bi-arrow-repeat" />}
                  >
                    {t('cal.sync_regenerate')}
                  </Button>
                  <span style={{ fontSize: 12, color: 'var(--color-text-secondary)' }}>
                    {t('cal.sync_regenerate_warn')}
                  </span>
                </div>
              </>
            );
