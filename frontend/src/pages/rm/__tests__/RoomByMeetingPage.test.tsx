...
import RoomByMeetingPage from '../RoomByMeetingPage';

vi.mock('react-router-dom', () => ({
  useParams: () => ({ uuid: 'room-uuid-1' }),
}));
...
vi.mock('../../../lib/http', () => ({
  httpPost: vi.fn().mockResolvedValue({
    ok: true,
    status: 200,
    json: async () => [{ name: 'Test Room', short_code: 'xyz789' }],
  }),
}));

vi.mock('../../../lib/api', () => ({
  getById: vi.fn().mockResolvedValue({ url: undefined }),
}));
