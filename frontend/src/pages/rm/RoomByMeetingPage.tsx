...
export default function RoomByMeetingPage() {
  const { uuid = '' } = useParams();

  const page = useModJoinPage({
    uuid,
    participantPathPrefix: '/r/',
    fetchInfo: async (id) => {
      const res = await httpPost('/api/pub/room/get', { id });
      if (!res.ok) throw new Error('not ok');
      const rows = await res.json();
      if (!rows[0]) throw new Error('no result');
      return { name: rows[0].label || rows[0].name || '', short_code: rows[0].short_code || '' };
    },
    fetchModeratorLink: async (id) => {
      const link = await getById('/api/pri/room/get/link', id);
      return link.url || undefined;
    },
    submitJoin: async (id, hostKey) => {
      const res = await httpPost('/api/pub/room/join/asmod/byroom', {
        room_id: id,
        host_key: hostKey,
      });
      ...
      const rows = await res.json();
      const row = rows[0];
      if (!row || row.error === 'invalid') throw new JoinPageError('invalid');
